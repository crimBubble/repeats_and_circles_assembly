...
for cl_number in cluster_dict:
    ...
    # walk subfolders in clustering and check for current CL
    # cur_dir = location, dirs = subfolders, files = files
    for cur_dir, dirs, files in os.walk(clustering_loc):
        for directory in dirs:
            ...
            if directory == dir_name:
                ...
                # target location
                target_loc = os.path.join(current_directory, "clust_assembly", scl_dir_name)
...
# walk subfolders in clust_assembly and merge reads
for cur_dir, dirs, files in os.walk(os.path.join(current_directory, "clust_assembly")):
    ...
    collected_fasta_list = os.path.join(cur_dir, "reads.scl" + scl_number + ".lst")
